...
void XsecCompWTError(){
	...
	Float_t** nDataB = new Float_t*[400];
	Float_t** nSel = new Float_t*[400];
	Float_t** nSelB = new Float_t*[400];
	Float_t** nGen = new Float_t*[400];
	Float_t** nGenB = new Float_t*[400];
	for(int ii=0; ii<400; ii++){
		nData[ii] = new Float_t[19];
		nSel[ii] = new Float_t[19];
		nGen[ii] = new Float_t[19];
		nDataB[ii] = new Float_t[19];
		nSelB[ii] = new Float_t[19];
		nGenB[ii] = new Float_t[19];
	...
	suffStat** nomEvtRateStat = new suffStat*[19];
	for(int ii=0; ii<19; ii++){
		nomEvtRateStat[ii] = new suffStat();
	}
	for(int iToy=0; iToy<nToys; iToy++){
		for(int ii=0; ii<19; ii++){
			nomEvtRateStat[ii]->Fill(nSel[iToy][ii]);
		}
	}
	...
	for(int iToy=0; iToy<nToys; iToy++){
		fitCov->Throw();
		for(int ii=0; ii<19; ii++){
			nDataB[iToy][ii]=((*priorVec)(binHelper->ips2full[ii])*nomEvtRateStat[ii]->GetMean()*(1.+(fitCov->varVec[binHelper->ips2full[ii]])));
		}
	}
	...
	//draw results
	DrawXsecWTErrorComp(nData,nDataB,nSel,nSelB,nGen,nGenB,binWidth,intFlux,nTargets,nTargetsNomMC,nToys);
